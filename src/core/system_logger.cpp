...
// ИСПРАВЛЕНО (деградация через часы работы): было 1024 строки. ESP32-S2 не имеет
// PSRAM, и кольцевой буфер на 1024 строки занимал до ~60-100 КБ кучи и постоянно
// переаллокировал String-и, фрагментируя кучу. Из-за этого через часы работы
// connecttohost() не мог выделить память и радио "переставало принимать станции".
// 192 строки достаточно для диагностики и занимают ~10-15 КБ.
sets::Logger webLogger(192);
