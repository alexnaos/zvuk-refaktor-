...
void handleWebRequests() {
    ...
    // ИСПРАВЛЕНО: WDT сбрасывается ПОСЛЕ sett.tick(), а не до.
    // Если sett.tick() зависнет на обработке большого JSON или записи на Flash,
    // WDT не будет сброшен и через 30 секунд произойдет аппаратная перезагрузка.
    // Теперь WDT кормится только после успешного выполнения tick().
    esp_task_wdt_reset();
}
